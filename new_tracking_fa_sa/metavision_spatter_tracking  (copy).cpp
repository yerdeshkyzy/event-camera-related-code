...
void Pipeline::tracker_callback(Metavision::timestamp ts,
    ...
    if (frame_generator_) {
        ...
        if(trackers.size()==2)
        {
            ...
            alldist = alldist+distance(trackers[0].get_centroid().x,trackers[0].get_centroid().y,trackers[1].get_centroid().x,trackers[1].get_centroid().y);
            allx = allx + trackers[i].get_centroid().x;
            ally = ally + trackers[i].get_centroid().y;

            centroids_arr[0][i] = trackers[i].get_centroid().x;
            centroids_arr[1][i] = trackers[i].get_centroid().y;
                
            alldist=alldist/(trackers.size()-1); // send 
        ...
        force_3d_msg.force_z = -3.36265431*pow(10,-5)*pow(force,5) + 1.13005289*pow(10,-2)*pow(force,4)-1.51567890*pow(10,0)*pow(force,3) + 1.01414076*pow(10,2)*pow(force,2) -3.38518586*pow(10,3)*pow(force,1) +  4.51004807*pow(10,4);
