...
// force 
    ...
    ros::Publisher fa_publisher = n.advertise<std_msgs::Int32>("fa_pub", 1000);
    ros::Publisher time_publisher = n.advertise<std_msgs::Float64>("time_pub", 1000);
...
void Pipeline::initialize_tracker() {
    ...
    camera_->cd().add_callback([this](const Metavision::EventCD *begin, const Metavision::EventCD *end) {
        ...
        for (const Metavision::EventCD *ev = begin; ev != end; ++ev) {
            if(ev->x >= 290 && ev->x <= 370 && ev->y >= 190 && ev->y <= 270 ){// kind of ROI for the central marker, counter is the number of events in a time window
        ...
        // fast afferent marker, the central one which is processed in raw method
        fa_msg.data = counter;
        fa_publisher.publish(fa_msg);
