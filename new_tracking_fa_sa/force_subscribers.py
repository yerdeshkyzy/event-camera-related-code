...
#####################################################
def rec():       
    global elapsed, data_all, data_row, counter, start, dateTimeObj, filename, weiss_force, ebts_force, delta_z, pixelxyz, desired_force, witt_force, centr_arr1, centr_arr2, centr_arr3, centr_arr4
    ...
    time_k = 100 # duration of data collection          
    ...
    if(counter > 0):
        ...
        if (elapsed <= time_k):  # some time period
            data_row = pd.concat( [pd.DataFrame(weiss_force), pd.DataFrame(ebts_force), pd.DataFrame(delta_z),  pd.DataFrame(pixelxyz), pd.DataFrame(desired_force), pd.DataFrame(centr_arr1),  pd.DataFrame(centr_arr2),  pd.DataFrame(centr_arr3),  pd.DataFrame(centr_arr4)], axis = 0)
