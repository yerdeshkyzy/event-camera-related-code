...
from metavision_designer_core import HalDeviceInterface, CdProducer, FrameGenerator, ImageDisplayCV, RoiFilter
...
def parse_args():  #function for opening the input file or live camera stream
    import argparse
...
def main():
    """Main function"""
    listener()
    global save_raw    #used for saving each raw when tactile exploration is true
    args = parse_args()

    from_file = False
    #the if statements below open the input file or else take the live camera data as input
    ...
    roi_filter = RoiFilter(prod_cd,x0=275,y0=165,x1=305,y1=195, reset_orig=True) #first marker area
    controller.add_component(roi_filter)

    # Generate a graphical representation of the events
    frame_generator = FrameGenerator(roi_filter)  # Or you can also specify the intensity for on/off events
    ...
    #bias settings
    bias_values = [299,221,384,1443,1401,1300,1500]   #[299,221,384,1593,1499,1250,1500]  original && medium
